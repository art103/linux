...
#define HI3670_TEMP(chan)		(HI3660_OFFSET(chan) + 0x1C)
#define HI3670_TH(chan)			(HI3660_OFFSET(chan) + 0x20)
#define HI3670_LAG(chan)		(HI3660_OFFSET(chan) + 0x28)
#define HI3670_INT_EN(chan)		(HI3660_OFFSET(chan) + 0x64)
#define HI3670_INT_CLR(chan)		(HI3660_OFFSET(chan) + 0x30)
...
static inline void hi3670_thermal_alarm_clear(void __iomem *addr,
					      int id)
{
	writel(1, addr + HI3670_INT_CLR(id));
	writel(0, addr + HI3670_INT_CLR(id));
}
...
static int hi3670_thermal_irq_handler(struct hisi_thermal_sensor *sensor)
{
	...
	hi3670_thermal_alarm_clear(data->regs, sensor->id);
	return 0;
}
...
static int hi3670_thermal_probe(struct hisi_thermal_data *data)
{
	...
	data->sensor[0].id = HI3670_LITTLE_SENSOR;
	data->sensor[0].irq_name = "tsensor_a53";
	data->sensor[0].data = data;

	if (data->nr_sensors > 1) {
		data->sensor[1].id = HI3670_BIG_SENSOR;
		data->sensor[1].irq_name = "tsensor_a73";
		data->sensor[1].data = data;
	}
